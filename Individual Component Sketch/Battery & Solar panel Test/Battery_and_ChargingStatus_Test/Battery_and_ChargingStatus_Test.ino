...
#define MIN_VOLTAGE  3.5   // Tensão considerada 0% (limite de descarga seguro aproximado)
#define MAX_VOLTAGE  5.5
...
void loop() { 
    ...
    if (chargeStatus == HIGH) {
       SerialUSB.println("Not Charging");       // HIGH = no charging (pode ser carga completa, sem bateria ou sem USB(painel) )        
    } else {
        
        SerialUSB.println("Charging");      // LOW = charging"
    } 
    delay(1000);
}